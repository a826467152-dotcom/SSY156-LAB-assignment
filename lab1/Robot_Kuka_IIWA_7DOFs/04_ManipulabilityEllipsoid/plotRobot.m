...
% u(9:10): plot view point, depends on axis_view_control, which can have
%           two values  0: (default) sliders do not affect the view, 
%                       1: the sliders control the camera view. 
% u(11)  : controls if the additional CF and EF markers should be visible
%           1: show additional markers, 0: no additional markers 
% u(12:18): robot joint velocities
...
%TODO: Finish the function manipulability_kuka_iiwa7
[HT_W,x7_W, x7p_w, w7p_w, w7_w,Heax_W,S,U,w,EAxis_W] = manipulability_kuka_iiwa7(Q,Qp,t,dt);
...
if(t==0)
    ...
    %% CF initialization
    cf_names={'cf_w','cf_0','cf_1','cf_2','cf_3','cf_4','cf_5','cf_6','cf_{ef}'};
    ...
    textHandler{1}=text(x7_W(1)+label_offset,x7_W(2)+label_offset,x7_W(3)+label_offset,tag, 'color',[0 0 0],'fontsize',18,'fontweight','b');
...
if(samples==vsr)
    ...
    if u(11)
        ...
        % labels of each text.
        textHandler{1}.Position=[x7_W(1)+label_offset,x7_W(2)+label_offset,x7_W(3)+label_offset];
...
Xout=[x7_W;x7p_w(1:3,1);w7p_w(1:3,1);w7_w(1:3,1)];
