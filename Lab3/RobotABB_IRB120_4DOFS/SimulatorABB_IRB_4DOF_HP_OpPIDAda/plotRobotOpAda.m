...
persistent robot1 iviz samples vsr state_s efMarker cfMarker number_links 
persistent cf_names fh xc yc zc hp
...
if(plot_robot)
    ...
    if(t==0)
        ...
        ax.YLim=workSpace;
        ax.ZLim=workSpace;
        ax.View=[160.0105,23.8770];
        ax.CameraViewAngle=2.7934;
        ax.CameraPosition=[9.4460   22.1799   10.5570];
        ax.CameraTarget=[0.2, 0, 0];
        ...
        xc=r*cos(2*pi*d')+cx;
        yc=r*sin(2*pi*d')+cy;
        zc=u(15)*ones(numel(d),1);

        hp=plot3(xc,yc,zc,'Color',[0, 0, 0], 'LineWidth',3);
        ...
        cf_names={'cf_w','cf_0','cf_1','cf_2','cf_3','ef_d'};
    ...
    if(samples==vsr)
        ...
        if u(7)
            ...
            for i=1:number_links-1
                updateCF(cfMarker{i},HT_W{i},lengthEF);
            end
            
            HTd_0=[eye(3) u(10:12); [0 0 0 1]];
            HTd_w=HT_W{1}*HTd_0;
            updateCF(cfMarker{number_links},HTd_w,lengthEF);
